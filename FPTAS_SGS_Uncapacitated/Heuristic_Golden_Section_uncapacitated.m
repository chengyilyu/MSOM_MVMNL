function [Heuristic_best_revenue, maximum_revenue_index, Heuristic_best_X] = Heuristic_Golden_Section_uncapacitated( )
    ...
    if L == 0
        ...
        if J == num_group_in_one_cluster*L
    ...
    fprintf('Running time of the heuristic algorithm: %f seconds\n', Heuristic_t_End);
...
%                 end
                    ...
                    [gamma_for_cluster_j,~] = calculate_gamma_Lambda_cluster (j, current_x_for_cluster);
                    log_total_gamma_bundle_upper = ceil(log(gamma_for_cluster_j)/log(1+epsilon_prime));
                    if log_total_gamma_bundle_upper > K-k
...
%                             end
                        ...
                        [temp_local_optimal_rev_for_this_cluster,current_x_for_cluster, update_flag] = find_better_policy_in_group_j(j,current_index,current_x_for_cluster,temp_local_optimal_rev_for_this_cluster,Gamma,K,k);
                        if update_flag == 0 
                    ...
                    %temp_local_optimal_rev_for_this_cluster = temp_local_optimal_rev_for_this_cluster + optimal_V(j+num_group_in_one_cluster, log_total_gamma_bundle+k+1);
                    if temp_local_optimal_rev_for_this_cluster > temp_optimal_rev 
                        temp_optimal_rev = temp_local_optimal_rev_for_this_cluster;
                        matrix_x{j,k+1} = current_x_for_cluster';
                        optimal_x_history{j,k+1} = [j + num_group_in_one_cluster, log_total_gamma_bundle_upper + k+1];
                    end
                optimal_V(j,k+1) = temp_optimal_rev;
            end
        end
        revenue_K = optimal_V(1,1);
        if revenue_K ~= -inf
            optimal_x = zeros(J,I);
            last_j = 1;
            last_k = 1;
            while (last_j <= J)
                if size(matrix_x{last_j,last_k},1) > 1
                    optimal_x(last_j:last_j+num_group_in_one_cluster-1,:) = matrix_x{last_j,last_k};
                else
...
function  [temp_local_optimal_rev_for_this_cluster,current_x_for_cluster, update_flag] = find_better_policy_in_group_j(j_begin,current_index,current_x_for_cluster,temp_local_optimal_rev_for_this_cluster,Gamma,K,k)
    global I utility_v0 optimal_V num_group_in_one_cluster epsilon_prime;
    
    temp_x_of_j = current_x_for_cluster(:,current_index);
    temp_x_of_cluster = current_x_for_cluster;
    update_flag = false;

    for i = 0 : I
        temp_x_of_cluster(:,current_index) = [ones(1,i),zeros(1,I-i)]';
        ...
        if (1 <= log_total_gamma_bundle_upper && log_total_gamma_bundle_upper <= K-k) 
            gamma_cluster_upper_bound = (1+epsilon_prime)^(log_total_gamma_bundle_upper);
            temp_revenue = temp_Lambda/(gamma_cluster_upper_bound * (1+ (utility_v0-1)/Gamma)) + optimal_V(j_begin+num_group_in_one_cluster, log_total_gamma_bundle_upper+k+1);
